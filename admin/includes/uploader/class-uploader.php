...
class WC_Dynamic_Gallery_Uploader extends WC_Dynamic_Gallery_Admin_UI
{
	...
	public function upload_input ( $name_attribute, $id_attribute = '', $value = '', $default_value = '', $field_name = '', $class = '', $css = '', $description = '', $post_id = 0 ) {
		...
		$output .= '<input id="upload_'.$id_attribute.'" class="a3rev-ui-upload-button a3_upload_button button" type="button" value="'.__( 'Upload', 'woo_dgallery' ).'" rel="'.$post_id.'" /> '.$description;
	...
	public function change_button_text( $translation, $original ) {
	    ...
	    if ( $original == 'Insert into Post' ) {
	    	$translation = __( 'Use this Image', 'woo_dgallery' );
			if ( isset( $_REQUEST['title'] ) && $_REQUEST['title'] != '' ) { $translation =__( 'Use as', 'woo_dgallery' ).' '.esc_attr( $_REQUEST['title'] ); }
	    }
	...
	public function modify_tabs ( $tabs ) {
		if ( isset( $tabs['gallery'] ) ) { $tabs['gallery'] = str_replace( 'Gallery', __( 'Previously Uploaded', 'woo_dgallery' ), $tabs['gallery'] ); }
		return $tabs;
	}
